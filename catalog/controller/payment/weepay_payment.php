...
class ControllerPaymentWeepayPayment extends Controller {
    ...
    public function index() {
        ...
        if (VERSION >= '2.2.0.0') {
            $template_url = 'payment/weepay_payment.tpl';
        } else {
            $template_url = 'default/template/payment/weepay_payment.tpl';
        }
    ...
    public function checkoutform() {
        try {
            ...
            $route_url = 'payment/weepay_payment/callback';
            $callback_url = $this->getSiteUrl() . 'index.php?route=' . $route_url;
            $order_id = $this->session->data['order_id'];
            $unique_conversation_id = (string) $order_id;
            $this->load->language('payment/weepay_payment');
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/weepay_payment.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/payment/weepay_payment.tpl';
            } else {
                $this->template = 'default/template/payment/weepay_payment.tpl';
            }
            $this->load->model('checkout/order');
